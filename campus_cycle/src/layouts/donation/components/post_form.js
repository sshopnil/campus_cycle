...
export default function PostForm({ open, onClose }) {
  const handleClose = () => {
    onClose(); // Call the onClose function passed from the parent component to close the dialog
  };
  ...
  return (
    <React.Fragment>
      <Dialog
        open={open}
        onClose={handleClose} // Use the handleClose function to handle dialog closure
        ...
        <AppBar sx={{ position: "relative" }}>
          <Toolbar>
            <IconButton
              edge="end"
              color="inherit"
              onClick={handleClose} // Close the dialog when close button is clicked
              aria-label="close"
            >
              <CloseIcon />
            </IconButton>
